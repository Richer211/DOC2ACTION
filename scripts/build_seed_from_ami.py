...
DEFAULT_INPUT = ROOT / "ml" / "data" / "external" / "ami" / "processed" / "ami.sample.jsonl"
DEFAULT_OUTPUT = ROOT / "ml" / "data" / "external" / "ami" / "processed" / "ami.seed.auto.jsonl"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build weak-label seed dataset from AMI transcript samples.")
...
def compose_input_text(row: dict[str, str]) -> str:
    ...
    prefix = f"Meeting: {meeting_id}\nSpeaker: {speaker}\n\n" if meeting_id or speaker else ""
    return f"{prefix}{transcript}".strip()
...
def main() -> None:
    ...
    with args.input.open("r", encoding="utf-8") as inp, args.output.open("w", encoding="utf-8") as out:
        for line in inp:
            ...
            prediction = analyze(AnalyzeRequest(text=input_text, document_type="meeting_notes")).model_dump()
            seed = {
                "id": row.get("id", ""),
    ...
    print(f"Converted {converted} AMI rows into seed file: {args.output}")
